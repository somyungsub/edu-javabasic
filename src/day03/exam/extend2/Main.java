...
public class Main {
    public static void main(String[] args) {
        ...
        // 요청사항 : 타원도 만들어주세요!!  ->  여기에 타원을 하위클래스 만들고 area를 구하는 로직을 추가해봅시다!
        // System.out.println(getArea(new Eclipse(new Point(10,10))));
    }

    /*
        이함수를 통해 동적바인딩의 흐름을 익히시기 바랍니다.
        - 상위클래스 Shape 는 하위클래스 Triangle, Rectangle를 포함할 수 있습니다. 즉 묵시적 형변환이 가능하여 받아 들일 수 있습니다.    -> 다형성
        - 매개변수로 들어온 하위클래스 타입에 따라 area 메서드가 호출하게 되는데 이런 흐름을 동적바인딩이라 합니다. -> 오버라이딩이 되어 있어야합니다.

        * 다형성, 오버라이딩을 통한 동적바인딩의 특징은 엄청 많이 사용되며 객체지향 프로그래밍의 핵심입니다.
        * 아래 함수 보시는 것처럼 코드 한줄이면 어떤 하위 클래스가 들어와도 그 하위클래스의 area() 작업을 수행하게 됩니다.
          -> 이개념을 통해 수정작업이 발생할 때 우리는 하위클래스의 내용만 수정을 하면 되고 함수 getArea는 손델 필요가 없게 됩니다.

          ex) 삼각형의 넓이를 1/2를 한번 더 해달라는 요청이 들어왔다면 우리는 Triangle 클래스의 area메서드만 수정을 하면됩니다.
